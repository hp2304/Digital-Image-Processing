...
//----------------------------------------------------------------------------------------------------
        ...
        comp (*psf_ft)[len] = malloc(sizeof(comp[len][len]));
        initZero2(&psf_ft[0][0], len);
        for(int i=0; i<BLUR_FACTOR; ++i){
                psf[i] = (1/(double)BLUR_FACTOR);
                psf_ft[0][i].real = psf[i];
                psf_ft[0][i].imag = 0.0;
        }
		...
		pthread_attr_t attr;
		pthread_attr_init(&attr);
		pthread_create(&tids[i], &attr, find_1d_fft, &psf_ft[i][0]);
	}
        for (int i = 0; i < len; i++) {
		pthread_join(tids[i], NULL);
	}
        transpose(&psf_ft[0][0], len, len);
        for (int i = 0; i < len; i++) {
		pthread_attr_t attr;
		pthread_attr_init(&attr);
		pthread_create(&tids[i], &attr, find_1d_fft, &psf_ft[i][0]);
...
//-----------------------------------------------------------------------------------------------------
        double nsr=0;
        printf("\nEnter noise to signal ratio : ");
        scanf("%lf", &nsr);
        
        for(int i=0; i<len; ++i){
                for(int j=0; j<len; ++j){
                        comp num = {0};
                        double abs_val = pow(psf_ft[i][j].real,2) + pow(psf_ft[i][j].imag,2);
                        num.real = psf_ft[i][j].real / (abs_val+nsr);
                        num.imag = -psf_ft[i][j].imag / (abs_val+nsr);
                        fft[i][j] = mult_comp_nums(fft[i][j], num);
                }
        }
